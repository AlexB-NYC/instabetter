...
const img = new HTMLImageElement(); img.tagName = 'IMG'; img.getAttribute = (name) => name === 'data-mime-type' ? '' : ''; img.currentSrc = 'https://cdn.example.com/image.jpg'; img.src = 'https://cdn.example.com/image.jpg';
assert.strictEqual(h.inferMediaExtension(img, 'https://cdn.example.com/picture.webp?x=1'), 'webp');
assert.strictEqual(h.normalizeMediaKind('img'), 'image');
assert.strictEqual(h.normalizeMediaKind('image'), 'image');
assert.strictEqual(h.normalizeMediaKind('IMG'), 'image');
assert.strictEqual(h.normalizeMediaKind(1), 'image');
assert.strictEqual(h.normalizeMediaKind('1'), 'image');
assert.strictEqual(h.normalizeMediaKind('video'), 'video');
assert.strictEqual(h.normalizeMediaKind('VIDEO'), 'video');
assert.strictEqual(h.normalizeMediaKind(2), 'video');
assert.strictEqual(h.normalizeMediaKind('2'), 'video');
assert.strictEqual(h.normalizeMediaKind(''), null);
assert.strictEqual(h.normalizeMediaKind(null), null);
assert.strictEqual(h.normalizeMediaKind(undefined), null);
assert.strictEqual(h.normalizeMediaKind('unknown'), null);
...
(async () => {
  ...
  const imageItem = await h.buildDownloadItemForSlide({ media: img, kind: 'image' }, { querySelectorAll: () => scripts, querySelector: () => null }, { username: 'user' }, '2026-07-20_00-00-00', 0, 1);
  assert.strictEqual(imageItem.sourceKind, 'image');
  assert.strictEqual(imageItem.url, 'https://cdn.example.com/image.jpg');
